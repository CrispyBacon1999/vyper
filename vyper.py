...
class API():
    ...
    def request(self, endpoint, parameters=None, file=None):
        ...
        # Make Request
        req = json.loads(requests.get(viperconfig.apiurl % (self._token, endpoint), params=parameters, files=file).text)
        if req['ok']:
            return req['result']
        else:
    ...
    def getUpdates(self):
        if os.path.isfile(viperconfig.lastupdatefile):
            lastupdate = open(viperconfig.lastupdatefile, 'r')
        else:
            lastupdate = open(viperconfig.lastupdatefile, 'a+')
        ...
        for update in updates:
            for key, value in self._functions.items():
                if key in update:
                    value(update)
        lastupdate = open(viperconfig.lastupdatefile, 'w')
    ...
    def sendInvoice(self, chat_id, title, description, payload, provider_token, start_parameter, currency, prices, photo_url='',
                    photo_size=None, photo_width=None, photo_width=None, need_name=False, need_phone_number=False, need_email=False, need_shipping_address=False,
